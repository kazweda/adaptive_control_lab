...
**パラメータ**
- プラント: `a=0.99`, `b=0.35`
- PIDゲイン: `Kp=1.305`, `Ki=0.65`, `Kd=0.0`

**期待される挙動**
- 約200ステップにわたって振動が継続する
- 振幅が徐々に減衰するが収束が非常に遅い
- システムが不安定化寸前の危険な状態を示す例
- オーバーシュート例と明確に区別できる長期振動

**注:** このシナリオは UI ゲイン上限 Kp = 1.5, Ki = 0.7 での検証を想定しています（Issue #54）。
...
**低周波のサイン波** (`sinusoid_slow`)
- 角周波数: ω = 0.05 rad/step
- 振幅: 0.2
- 用途: ゆっくりとした周期的な外乱への追従性能を確認

**高周波のサイン波** (`sinusoid_fast`)
- 角周波数: ω = 0.5 rad/step
- 振幅: 0.15
- 用途: 速い周期的な外乱への追従性能を確認
