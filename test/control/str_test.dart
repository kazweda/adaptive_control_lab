...
void main() {
  group('STR (Self-Tuning Regulator)', () {
    ...
    group('制御則計算（2次系）', () {
      test('基本的な制御入力計算', () {
        // y=0.5, y(k-1)=0.3, u(k-1)=0.2
        // r=1.0, a1=0.8, a2=-0.3, b1=0.5, b2=0.1
        // p1=0.5, p2=0.3
        // u = (1/b1) * [r - (a1 - p1 - p2)*y - (a2 - p1*p2)*y(k-1) - b2*u(k-1)]
        //   = (1/0.5) * [1.0 - (0.8 - 0.8)*0.5 - (-0.3 - 0.15)*0.3 - 0.1*0.2]
        //   = 2 * [1.0 - 0 + 0.135 - 0.02]
        //   = 2 * 1.115 = 2.23
        final u1 = str2.computeControl(0.5, 1.0);
        final u2 = str2.computeControl(0.3, 1.0);
        // 初回と2回目で過去値が異なるため動作確認
        expect(u1, isNotNull);
        expect(u2, isNotNull);
      });
